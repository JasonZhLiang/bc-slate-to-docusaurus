...
An alternative method is to call the update method on a separate thread.Â This has the advantage that the Â UI will never hang if a callback takes a long time. The downside is that interactions with the UI of your app will need to be marshaled to the main application thread.
```java
